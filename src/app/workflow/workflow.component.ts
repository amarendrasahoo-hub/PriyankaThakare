...
export class WorkflowComponent implements OnInit {
  ...
  ngOnInit() {
    this.workflowService.getWorkflow("1234").subscribe(
      (data) => {
        this.MyDataSource = new MatTableDataSource();
        this.myWorkflows = data;
        console.log(
          "type of newdatasource",
          typeof this.newdatasource,
          typeof this.dataSource
        );
  ...
  redirectToDetails(element) {
    ...
    this.workFlowDialogRef.afterClosed().subscribe((result) => {
      ...
      if (this.action == "A") {
        ...
        this.workflowService.updateWorkflow(workflowdata).subscribe(
          (resp: WorkflowResponse) => {
            console.log(resp);
            if (resp.completed == true) {
              this.ApprovalMessage =
                "Wrokflow " + resp.workflowId + " is approved by You";
            } else {
              this.ApprovalMessage =
                "Wrokflow " +
                resp.workflowId +
                " is approved by You..! The next Approver is " +
                resp.nextApprover;
            }
          },
          (error: any) => {
