  ...
  onSubmit() {
    if (this.usbaccessForm.invalid) {
      ...
      var days = this.calculateDays(
        this.usbaccessForm.value.sdate,
        this.usbaccessForm.value.edate
      );
      var newstring = this.concatenateArrayString(
        this.usbaccessForm.value.accessfor
      );
      ...
      this.usbaccessserviceobj.postUsbAccessReq(usbAccessReq).subscribe(
        (data: PostResponse) => {
          const resData = data;
